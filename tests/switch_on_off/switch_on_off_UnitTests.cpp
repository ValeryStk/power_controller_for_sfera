...
void switch_on_off_UnitTests::switch_on_off_test() {
    ...
    for (int i = 0; i < lamps.size(); ++i) {
        ...
        QString text_message =
            QString(tlc::kOperationSwitchOffResultFailed).arg(voltage);
    ...
    for (int i = 0; i < lamps.size(); ++i) {
        ...
        auto current_limit =
            lamps[i].toObject()[global::kJsonKeyMaxCurrent].toDouble();
        auto diff = current_limit - current;
        QString text_message =
            QString(tlc::kOperationSwitchOnResultFailed).arg(current);
        auto message = text_message.toStdString();
