...
PowerSupplyManager::PowerSupplyManager() {
    ...
    m_IpAddresses << "192.168.1.24"
                  << "192.168.1.23"
                  << "192.168.1.22";  //<<"192.168.1.25";
    m_hostAddress.setAddress(m_IpAddresses.at(m_powerSupplyIndex));
    m_hostPort = 9221;
    m_isTimeOut = false;
    timeOutTimer.setInterval(5000);
    initializeJSonObjects();
    powers_params = {{PowerOuts::POW1_OUT1, {0, 0, 0, false}},
    ...
    connect(&increaseVoltageTimer, SIGNAL(timeout()), this,
            SLOT(increasingVoltage()));
    connect(&testDisplayTimer, SIGNAL(timeout()),
            SLOT(testChangeRandomAllParams()));
    connect(&decreasingVoltageTimer, SIGNAL(timeout()), this,
            SLOT(decreasingProcess()));
...
const testResults &PowerSupplyManager::results() const { return m_results; }
...
void PowerSupplyManager::initializeJSonObjects() {
    ...
    if (!file.exists()) {
        QJsonObject outObj{
            {"isOn", false},          {"Voltage", 0.0},
            {"Current_T", 0.0},       {"LimitCurrent", 0.0},
            {"LimitVoltage", 0.0},    {"Power", 0.0},
            {"Delta_V", 0.0},         {"Delta_t", 0.0},
            {"TargetName", "Lamp X"},
        };
        ...
        QJsonArray jArray = {powerSupply_1, powerSupply_2, powerSupply_3,
                             powerSupply_4};
        ...
        qDebug() << "Check ps1 property: "
                 << powerSupply_1.value("PowerSupplyName").toString();
        qDebug() << "Check ps2 property: "
                 << powerSupply_2.value("PowerSupplyName").toString();
        qDebug() << "Check ps3 property: "
                 << powerSupply_3.value("PowerSupplyName").toString();
        // qDebug()<<"Check ps4 property:
        // "<<powerSupply_4.value("PowerSupplyName").toString();

        for (int i = 0; i < m_IpAddresses.count(); ++i) {
            double pl1 = doc.array()
                             .at(i)
                             .toObject()
                             .value("out1")
                             .toObject()
            ...
            double pl2 = doc.array()
                             ...
                             .value("Current_target")
                             .toDouble();
            m_powerLimits.insert(QPair<QString, int>(m_IpAddresses.at(i), 1),
                                 pl1);
            m_powerLimits.insert(QPair<QString, int>(m_IpAddresses.at(i), 2),
                                 pl2);
            qDebug() << "-----> pL: " << pl1 << pl2;
        }
    }
...
void PowerSupplyManager::getVoltage(quint16 unit) {
    ...
    if (unit == 1) parametr_state = parametrRead::V1;
    if (unit == 2) parametr_state = parametrRead::V2;
...
void PowerSupplyManager::getCurrentLimit(quint16 unit) {
    ...
    if (unit == 1) parametr_state = parametrRead::LI1;
    if (unit == 2) parametr_state = parametrRead::LI2;
...
void PowerSupplyManager::getCurrentValue(quint16 unit) {
    ...
    if (unit == 1) parametr_state = parametrRead::I1;
    if (unit == 2) parametr_state = parametrRead::I2;
...
void PowerSupplyManager::getPowerStatus(quint16 unit) {
    ...
    if (unit == 1) parametr_state = parametrRead::S1;
    if (unit == 2) parametr_state = parametrRead::S2;
        ...
        if (operation_variant == operation::increaseVoltage) {
            ...
            switch (parametr_state) {
                case PowerSupplyManager::parametrRead::ID:
                    break;
                case parametrRead::S1:
                    qDebug() << "Power out 1 state: " << answer;
                    break;
                case parametrRead::S2:
                    qDebug() << "Power out 2 state: " << answer;
                    break;
                ...
                case parametrRead::LI1:
                    qDebug() << "Current out 1: " << answer;
                    break;
                case parametrRead::LI2:
                    qDebug() << "Current out 2: " << answer;
                    break;
            }
        }
        if (operation_variant == operation::test) {
            ...
            switch (parametr_state) {
                ...
                case PowerSupplyManager::parametrRead::V1:
                    qDebug() << "V1: " << answer;
                    setParamsForPwrSupply(out::out1, parametrRead::V1,
                                          answer.toDouble());
                    getVoltage(2);
                    break;
                case PowerSupplyManager::parametrRead::V2:
                    qDebug() << "V2: " << answer;
                    setParamsForPwrSupply(out::out2, parametrRead::V2,
                                          answer.toDouble());
                    getCurrentValue(1);
                    break;
                case PowerSupplyManager::parametrRead::I1:
                    qDebug() << "I1: " << answer;
                    setParamsForPwrSupply(out::out1, parametrRead::I1,
                                          answer.toDouble());
                    getCurrentValue(2);
                    break;
                case PowerSupplyManager::parametrRead::I2:
                    qDebug() << "I2: " << answer;
                    setParamsForPwrSupply(out::out2, parametrRead::I2,
                                          answer.toDouble());
                    getCurrentLimit(1);
                    break;
                case PowerSupplyManager::parametrRead::LI1:
                    qDebug() << "LI1: " << answer;
                    setParamsForPwrSupply(out::out1, parametrRead::LI1,
                                          answer.toDouble());
                    getCurrentLimit(2);
                    break;
                case PowerSupplyManager::parametrRead::LI2:
                    qDebug() << "LI2: " << answer;
                    setParamsForPwrSupply(out::out2, parametrRead::LI2,
                                          answer.toDouble());
                    getPowerStatus(1);
                    break;
                case PowerSupplyManager::parametrRead::S1:
                    qDebug() << "PS1: " << answer;
                    setParamsForPwrSupply(out::out1, parametrRead::S1,
                                          answer.toDouble());
                    getPowerStatus(2);
                    break;
                case PowerSupplyManager::parametrRead::S2:
                    qDebug() << "PS2: " << answer;
                    setParamsForPwrSupply(out::out2, parametrRead::S2,
                                          answer.toDouble());
                    qDebug() << "********************************\n\n";
                    switchOnAllUnits();
                    m_socket->disconnectFromHost();
            ...
            switch (parametr_state) {
                case parametrRead::V1: {
                    ...
                    if (psV > 0.005) {
                        ...
                        if (m_decreaseV < 0) m_decreaseV = 0;
...
void PowerSupplyManager::timeOutCase() {
    ...
    qDebug() << "Timeout";
    timeOutTimer.stop();
    emit timeOutCaseWasHappened();
    qDebug() << "Timeout notfications emitted...........";
    m_sounder.playSound("timeOutCase.mp3");
}

void PowerSupplyManager::increasingVoltageFinished() {
    qDebug()
        << "Increasing voltage process is finished. +++++++++++++++++++++++";
    increaseVoltageTimer.stop();
...
void PowerSupplyManager::replaceUselessGetV(double &V, QString &msg) {
    if (msg.contains("V1")) msg.remove('\r').remove('\n').replace("V1 ", "");
    if (msg.contains("V2")) msg.remove('\r').remove('\n').replace("V2 ", "");
...
void PowerSupplyManager::checkIfPwrSupplyIsLoaded(const double &V,
                                                  const double &I) {
    if (V >= m_check_I) {
        if (I < m_load_I) {
...
void PowerSupplyManager::setParamsForPwrSupply(out out, parametrRead pr,
                                               const double &value) {
    QString outStr;
    switch (out) {
        case PowerSupplyManager::out::out1:
            outStr = "out1";
            break;
        case PowerSupplyManager::out::out2:
            outStr = "out2";
            break;
    }
    QJsonObject power;
    if (m_powerSupplyIndex == 0) power = powerSupply_1.value(outStr).toObject();
    if (m_powerSupplyIndex == 1) power = powerSupply_2.value(outStr).toObject();
    if (m_powerSupplyIndex == 2) power = powerSupply_3.value(outStr).toObject();
    if (m_powerSupplyIndex == 3) power = powerSupply_4.value(outStr).toObject();
...
double PowerSupplyManager::getCurrentLimitForCurrentPowerSupplyAndOut() {
    return m_powerLimits.value(
        QPair<QString, int>(m_IpAddresses.at(m_powerSupplyIndex), m_outNumber));
}
...
void PowerSupplyManager::connectedCase() {
    ...
    qDebug() << "Power supply " << m_IpAddresses.at(m_powerSupplyIndex)
             << " is connected.";

    if (operation_variant == operation::test) {
        setVoltage(1, 0);
        setVoltage(2, 0);
        setCurrentLimit(1, m_powerLimits.value(QPair<QString, int>(
                               m_IpAddresses.at(m_powerSupplyIndex), 1)));
        setCurrentLimit(2, m_powerLimits.value(QPair<QString, int>(
                               m_IpAddresses.at(m_powerSupplyIndex), 2)));
        getVoltage(m_outNumber);
    }
    if (operation_variant == operation::increaseVoltage) {
    ...
    if (operation_variant == operation::decreasingVoltageAndSwitchOff) {
        qDebug() << "Change network interface after lamp was switched off"
                 << m_powerSupplyIndex << m_outNumber;
        emit oneLampWasSwitchedOff();
    }
}
...
void PowerSupplyManager::disconnectedCase() {
    ...
    if (operation_variant == operation::decreasingVoltageAndSwitchOff) {
        if (m_powerSupplyIndex > 0) {
            ...
            // if all lamps were switched off
            emit oneLampWasSwitchedOff();
        }
    }
}
