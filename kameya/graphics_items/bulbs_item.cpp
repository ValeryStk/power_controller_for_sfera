...
BulbsQGraphicsItem::BulbsQGraphicsItem() {
    ...
    for (int i = 0; i < NUMBER_OF_LAMPS; ++i) {
        bulb_colors[i] =
            QColor(lamps[i].toObject()[global::kJsonKeyColor].toString());
...
void BulbsQGraphicsItem::drawLamps(QPainter *painter) {
    ...
    for (int i = 0; i < NUMBER_OF_LAMPS; ++i) {
        ...
        // без обводки
        int centerX = imgOn.width() / 2;
        int centerY = y + imgOn.height() / 2;
        ...
        painter->drawEllipse(
            QPoint(centerX + offsetX - 10, centerY + offsetY - 10), 8, 8);
